...
from generate_comm_from_graph import write_comm_csv
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            ...
            "graph_comm.ipynb と同じ principled_clustering で comm.csv を作ります。"
        )
    )
    ...
    parser.add_argument("--output-subdir", default="powerlaw_node_count")
    ...
    for num_nodes in args.num_nodes:
        for level in levels:
            for seed_index in args.seeds:
                network_name = f"pnc_n{num_nodes}_{level.name}_seed{seed_index}"
