...
OUTPUT_ROOT="experiments/2026-05-26_powerlaw_node_count_ba_comm/strategy_runs"
...
shopt -s nullglob
NETWORK_CONFIGS=("$NETWORK_CONFIG_DIR"/network-pncba_n*.toml)
shopt -u nullglob

if [ "${#NETWORK_CONFIGS[@]}" -eq 0 ]; then
  echo "Error: network-pncba_n*.toml が見つかりません。先に以下を実行してください。"
  echo "  .venv/bin/python scripts/prepare_powerlaw_node_count_ba_comm_experiment.py"
  exit 1
fi
...
echo "=== Start powerlaw-node-count-ba-comm strategy batch ===" | tee -a "$MASTER_LOG"
