...
OUTPUT_ROOT="experiments/2026-06-02_lfr_facebook_pool/strategy_runs"
...
shopt -s nullglob
NETWORK_CONFIGS=("$NETWORK_CONFIG_DIR"/network-lfrpool_*.toml)
shopt -u nullglob

if [ "${#NETWORK_CONFIGS[@]}" -eq 0 ]; then
  echo "Error: network-lfrpool_*.toml not found. Run:"
  echo "  .venv/bin/python scripts/prepare_lfr_facebook_pool_experiment.py"
  exit 1
fi
...
echo "=== Start lfr-facebook-pool strategy batch ===" | tee -a "$MASTER_LOG"
