...
OUTPUT_ROOT="experiments/2026-06-03_lfr_rust_target_pool/strategy_runs"
NETWORK_CONFIG_DIR="v2/test_2/network"

if [ ! -x "$RUST_BIN" ]; then
  echo "Error: $RUST_BIN is missing or not executable. Run cargo build --release first."
  exit 1
fi

shopt -s nullglob
NETWORK_CONFIGS=("$NETWORK_CONFIG_DIR"/network-lfrrustpool_*.toml)
shopt -u nullglob

if [ "${#NETWORK_CONFIGS[@]}" -eq 0 ]; then
  echo "Error: network-lfrrustpool_*.toml not found. Run:"
  echo "  .venv/bin/python scripts/prepare_lfr_rust_target_pool_experiment.py"
  exit 1
fi
...
echo "=== Start lfr-rust-target-pool strategy batch ===" | tee -a "$MASTER_LOG"
