...
OUTPUT_ROOT="experiments/2026-05-19_powerlaw_degree_cluster/strategy_runs"
...
shopt -s nullglob
NETWORK_CONFIGS=("$NETWORK_CONFIG_DIR"/network-pdc_*.toml)
shopt -u nullglob

if [ "${#NETWORK_CONFIGS[@]}" -eq 0 ]; then
  echo "Error: network-pdc_*.toml が見つかりません。先に以下を実行してください。"
  echo "  .venv/bin/python scripts/prepare_powerlaw_degree_cluster_experiment.py"
  exit 1
fi
...
echo "=== Start powerlaw-degree-cluster strategy batch ===" | tee -a "$MASTER_LOG"
