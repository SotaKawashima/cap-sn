...
DEFAULT_LEVELS = [
    ClusterLevel("low", 0.0),
    ClusterLevel("mid", 0.3),
    ClusterLevel("high", 1.0),
]
...
def write_network_config(path: Path, network_dir_name: str) -> None:
    path.write_text(
        "\n".join(
            [
                f'path = "./powerlaw_cluster/{network_dir_name}/"',
                'graph = "edgelist.txt"',
                "directed = false",
                "transposed = false",
                'community = "comm.csv"',
...
def parse_levels(raw_levels: list[str] | None) -> list[ClusterLevel]:
    if not raw_levels:
    ...
    for raw in raw_levels:
        try:
            ...
            levels.append(ClusterLevel(name=name, p=float(value)))
        except ValueError as exc:
            raise argparse.ArgumentTypeError(
                f"--level は name:p 形式で指定してください: {raw}"
            ) from exc
    return levels
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Powerlaw cluster graph を生成し、既存シミュレータ用の"
            "ネットワーク実験セットを用意します。"
        )
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=10,
        help="NetworkX powerlaw_cluster_graph の m。平均次数は約 2m。",
    )
    parser.add_argument("--seeds", type=int, nargs="+", default=[1, 2, 3])
    parser.add_argument(
        "--level",
        action="append",
        help="クラスタ水準を name:p 形式で指定。例: --level low:0.0",
    )
    parser.add_argument("--seed-base", type=int, default=20260518)
    parser.add_argument("--max-retries", type=int, default=100)
    parser.add_argument("--output-subdir", default="powerlaw_cluster")
    parser.add_argument(
        "--community",
        type=Path,
