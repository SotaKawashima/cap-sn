...
shopt -s nullglob
NETWORK_CONFIGS=("$NETWORK_CONFIG_DIR"/network-pnc_n*.toml)
shopt -u nullglob

if [ "${#NETWORK_CONFIGS[@]}" -eq 0 ]; then
  echo "Error: network-pnc_n*.toml が見つかりません。先に以下を実行してください。"
  echo "  .venv/bin/python scripts/prepare_powerlaw_node_count_experiment.py"
  exit 1
fi
...
echo "=== Start powerlaw-node-count strategy batch ===" | tee -a "$MASTER_LOG"
