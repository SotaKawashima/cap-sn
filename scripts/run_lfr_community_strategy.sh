...
OUTPUT_ROOT="experiments/2026-06-02_lfr_community/strategy_runs"
...
shopt -s nullglob
NETWORK_CONFIGS=("$NETWORK_CONFIG_DIR"/network-lfr_*.toml)
shopt -u nullglob

if [ "${#NETWORK_CONFIGS[@]}" -eq 0 ]; then
  echo "Error: network-lfr_*.toml が見つかりません。先に以下を実行してください。"
  echo "  .venv/bin/python scripts/prepare_lfr_community_experiment.py"
  exit 1
fi
...
echo "=== Start lfr-community strategy batch ===" | tee -a "$MASTER_LOG"
